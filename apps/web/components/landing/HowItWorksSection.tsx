...
const steps = [
  {
    ...
    codeSnippet: 'Create a responsive dashboard layout with a sidebar menu, header, and a main content area containing 3 stats cards and a chart.',
  },
  {
    id: 2,
...
export const HowItWorksSection = () => {
  ...
  return (
    <section id="how-it-works" className="py-24 relative z-10 w-full max-w-7xl mx-auto px-6">
      ...
      <div className="flex flex-col lg:flex-row gap-12 items-start">
        ...
        <div className="w-full lg:w-1/3 flex flex-col relative">
           ...
           <div className="absolute left-[39px] top-12 bottom-12 w-[2px] bg-white/10 hidden lg:block z-0">
             <motion.div 
               className="w-full bg-gradient-to-b from-lynx-primary to-lynx-accent"
               initial={{ height: "0%" }}
           ...
           {steps.map((step) => {
             ...
             return (
               ...
               >
                 <div className={`absolute left-0 top-7 w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-500 z-10
                 ...
                 <div className="relative z-10 bg-black/40 backdrop-blur-sm p-4 -ml-4 rounded-xl border border-transparent hover:border-white/5 transition-colors">
                   <h3 className={`text-2xl font-semibold mb-2 transition-colors ${isActive ? "text-white" : "text-zinc-400"}`}>
                     {step.title}
                   </h3>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {activeStep === 2 && (
                    <pre className="text-zinc-300 whitespace-pre-wrap">
                      <code dangerouslySetInnerHTML={{
                          __html: steps[1]!.codeSnippet.replace(/\+/g, '<span class="text-emerald-400">+</span>').replace(/>/g, '<span class="text-lynx-accent">></span>')
                      }} />
                    </pre>
                  )}
                  ...
                  {activeStep === 3 && (
                    <pre className="text-zinc-300 whitespace-pre-wrap">
                       <code dangerouslySetInnerHTML={{
                          __html: steps[2]!.codeSnippet.replace(/\$/g, '<span class="text-lynx-primary">$</span>').replace(/>/g, '<span class="text-lynx-accent">></span>').replace(/🟢/g, '<span class="animate-pulse inline-block">🟢</span>')
                      }} />
                    </pre>
                  )}
                </motion.div>
