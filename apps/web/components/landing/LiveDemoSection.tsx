...
import { Sparkles, ArrowRight, LayoutDashboard, Settings, UserCircle, Bell } from "lucide-react";

export const LiveDemoSection = () => {
  const [isGenerating, setIsGenerating] = useState(false);
  const [showResult, setShowResult] = useState(false);
  const [promptText, setPromptText] = useState("");
  const fullPrompt = "Create a modern settings dashboard with a sidebar menu, user profile section, and notification preferences toggles. Use a dark theme with glassmorphism.";
  ...
  return (
    <section className="py-24 relative w-full max-w-7xl mx-auto px-6">
      {/* Decorative Glows */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-lynx-primary/5 blur-[120px] rounded-full pointer-events-none z-[-1]" />
      ...
      <div className="text-center mb-16">
        ...
        <p className="text-zinc-400">Experience the speed of AI-driven UI creation.</p>
      </div>
      ...
      <div className="glass-panel rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 h-[600px]">
          ...
          <div className="p-8 border-b lg:border-b-0 lg:border-r border-white/10 flex flex-col justify-between bg-black/20">
            <div>
              ...
              <div className="relative group">
                {/* Focus glow effect */}
                <div className={`absolute -inset-0.5 bg-gradient-to-r from-lynx-primary/50 to-lynx-accent/50 rounded-xl blur opacity-0 transition duration-500 ${isGenerating && !showResult ? 'opacity-100' : 'group-hover:opacity-30'}`} />
                
                <div className="relative bg-zinc-900/80 border border-white/10 rounded-xl p-4 min-h-[150px] cursor-text">
                  <p className="text-zinc-300 min-h-[100px] font-medium leading-relaxed">
                    {promptText}
                    {isGenerating && <span className="inline-block w-2 h-4 bg-lynx-primary ml-1 animate-pulse translate-y-1" />}
                    {!isGenerating && !showResult && promptText.length === 0 && (
                      <span className="text-zinc-600">Describe the UI you want to build...</span>
                    )}
                  </p>
                </div>
              </div>
          ...
          <div className="relative bg-[#0c0c0e] overflow-hidden flex items-center justify-center p-6">
            <div className="absolute inset-0 pattern-grid-lg text-white/[0.02]" />
              ...
              {isGenerating && !showResult && (
                ...
                >
                  ...
                  <div className="flex flex-col items-center text-sm font-mono text-zinc-400 gap-1">
                    <span className="animate-pulse">&gt; Compiling components...</span>
                    <span className="animate-pulse delay-75">&gt; Generating styles...</span>
                  </div>
                </motion.div>
              )}
                ...
                >
                  ...
                  <div className="w-48 bg-black border-r border-white/5 p-4 flex flex-col gap-4">
                     ...
                     <div className="flex flex-col gap-2">
                       {['Profile', 'Account', 'Appearance', 'Notifications'].map((item, i) => (
                         <div key={item} className={`px-3 py-2 rounded-md text-sm ${i === 3 ? 'bg-white/10 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>
                           {item}
                         </div>
                       ))}
                     </div>
                  ...
                  <div className="flex-1 p-6 bg-zinc-950/50 flex flex-col gap-6">
                    <div className="flex items-center justify-between pb-4 border-b border-white/5">
                      ...
                      <div className="flex gap-2">
                        <button className="p-2 rounded-md bg-white/5 hover:bg-white/10"><Bell className="w-4 h-4" /></button>
                    ...
                    <div className="flex flex-col gap-4">
                      {[
                        { title: 'Email Alerts', desc: 'Receive daily summary emails' },
                        { title: 'Push Notifications', desc: 'Instant alerts on desktop' },
                        { title: 'Slack Integration', desc: 'Send alerts to #dev-team' },
                      ].map((setting, i) => (
                        <div key={i} className="flex items-center justify-between p-4 rounded-lg bg-white/5 border border-white/5">
                          <div>
                            <div className="font-medium text-sm text-zinc-200">{setting.title}</div>
                            <div className="text-xs text-zinc-500">{setting.desc}</div>
                          </div>
                          <div className={`w-10 h-6 rounded-full p-1 transition-colors ${i < 2 ? 'bg-lynx-primary' : 'bg-zinc-700'}`}>
                            <div className={`w-4 h-4 rounded-full bg-white transition-transform ${i < 2 ? 'translate-x-4' : 'translate-x-0'}`} />
                          </div>
                        </div>
                      ))}
                    </div>
